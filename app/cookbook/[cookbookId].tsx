...
export default function CookbookDetailScreen(): React.ReactElement {
	...
	const suggestedRecipes = useQuery(api.discoverFeed.getUnviewedRecipes, {
		limit: 1
	});

	const { openModal } = useAddModal();
	const saveToCookbookMutation = useMutation(api.discoverFeed.saveToCookbook);
	const getOrCreateRecipeMutation = useMutation(
		api.discoverFeed.getOrCreateRecipe
	);
	...
	const [optionsRecipeId, setOptionsRecipeId] =
	...
	const panGesture = Gesture.Pan()
		...
		.onUpdate((event) => {
			...
			if (proposed > maxTranslate) {
				translateX.value =
					maxTranslate + (proposed - maxTranslate) * 0.3;
			} else if (proposed < minTranslate) {
				translateX.value =
					minTranslate + (proposed - minTranslate) * 0.3;
			} else {
	...
	const handleSuggestedRecipePress = useCallback(
		async (discoverRecipeId: Id<'discoverRecipes'>) => {
			try {
				const recipeId = await getOrCreateRecipeMutation({
					discoverRecipeId
				});
				router.push(`/recipe/${recipeId}`);
			} catch {
	...
	const handleRecipeMorePress = useCallback(
		(recipeId: Id<'recipes'>, title: string) => {
					...
					{recentlyCooked ? (
						<View style={styles.sectionContainer}>
							<Text style={styles.sectionTitle}>
								{COPY.cookbookDetail.recentlyCooked}
							</Text>
							<View style={styles.recentCardWrapper}>
								<RecentCookCard
									...
									totalTimeMinutes={
										recentlyCooked.totalTimeMinutes ?? 0
									}
									difficulty={parseDifficulty(
										recentlyCooked.difficulty
									)}
									cuisine={recentlyCooked.cuisine}
									onPress={() =>
										handleRecipePress(recentlyCooked._id)
									}
									onCook={() =>
										handleCookPress(recentlyCooked._id)
									}
								/>
							</View>
						</View>
						...
						<View style={styles.sectionContainer}>
							...
							<View style={styles.recentCardWrapper}>
								<RecentCookCard
									...
									totalTimeMinutes={
										suggestedRecipes[0].totalTimeMinutes ??
										0
									}
									difficulty={parseDifficulty(
										suggestedRecipes[0].difficulty
									)}
									cuisine={suggestedRecipes[0].cuisine}
									onPress={() =>
										handleSuggestedRecipePress(
											suggestedRecipes[0]._id
										)
									}
									onCook={() =>
										handleAddSuggestedRecipe(
											suggestedRecipes[0]._id
										)
									}
